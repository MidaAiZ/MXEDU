    ...
    return {
        ...
        init: function () {
            ...
            function editRow(oTable, nRow, id) {
				...
				if (id) $(jqTds[3]).find("a.edit").attr("data-id", id);
            ...
            function saveRow(oTable, nRow, id) {
				...
				type = id ? 'PATCH' : "POST";
				url = id ? ('/manage/post_cates/' + id) : '/manage/post_cates';

				$.ajax({
					url:  url,
					data: {
						'post_cate[name]': jqInputs[0].value
					},
					type: type,
					dataType: 'JSON',
					success: function(res) {
						oTable.fnUpdate(res.name, nRow, 0, false);
						oTable.fnUpdate(res.posts_count, nRow, 1, false);
            ...
            $('#editable-sample').on('click', 'a.delete', function (e) {
				...
				$.ajax({
					url: '/manage/post_cates/' + $(this).data("id"),
					type: 'delete',
					dataType: 'JSON',
					success: function(res) {
						oTable.fnDeleteRow(nRow);
