    ...
    return {
        ...
        init: function () {
            ...
            function saveRow(oTable, nRow, id) {
				...
				type = id ? 'PATCH' : "POST";
				url = id ? ('/manage/product_cates/' + id) : '/manage/product_cates';

				$.ajax({
					url:  url,
					data: {
						'product_cate[name]': jqInputs[0].value
					},
					type: type,
					dataType: 'JSON',
					success: function(res) {
						oTable.fnUpdate(res.name, nRow, 0, false);
						oTable.fnUpdate(res.products_count, nRow, 1, false);
            ...
            $('#editable-sample').on('click', 'a.delete', function (e) {
				...
				$.ajax({
					url: '/manage/product_cates/' + $(this).data("id"),
					type: 'delete',
					dataType: 'JSON',
					success: function(res) {
						oTable.fnDeleteRow(nRow);
