...
(function( $ ) {
    ...
    $.fn.extend({
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
            ...
            if( opt.uploadButton ) {
				var uploadButton = opt.uploadButton;
				// delete opt.uploadButton;
			}
			...
			if ( opt.complete ) {
				var completeCallback = opt.complete;
				delete opt.complete;
			}
	...
	function createBox( $fileInput, file, webUploader, opt ) {
        ...
        var $parentFileBox = $fileInput.parents('.parentFileBox');
		...
		if ( $parentFileBox.find('.diyButton').length <= 0 ) {
            if (opt.uploadButton) {
                var $startButton = opt.uploadButton;
            } else {
		...
		var li = '<li id="fileBox_'+file_id+'" class="diyUploadHover"> \
					...
					<div class="diyCancel"><i class="fa fa-times-circle"></i></div> \
		...
		$parentFileBox.children('.fileBoxUl').find('.upload-btn').before( li );
